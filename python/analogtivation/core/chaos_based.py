...
def logistic_map_activation(
    x: Union[np.ndarray, float], r: float = 3.9, iterations: int = 3
) -> Union[np.ndarray, float]:
    """Functional interface for logistic map activation."""
    return LogisticMapActivation(r, iterations).forward(x)


def mandelbrot_activation(
    x: Union[np.ndarray, float], max_iter: int = 5, escape_radius: float = 2.0
) -> Union[np.ndarray, float]:
