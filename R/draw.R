...
draw_clock <- function(time = NULL, show_activation = FALSE) {
  ...
  hands_df <- data.frame(
    hand = c("hour", "minute"),
    x_end = c(0.5 * cos(hour_angle * pi/180), 
              0.8 * cos(minute_angle * pi/180)),
    y_end = c(0.5 * sin(hour_angle * pi/180), 
              0.8 * sin(minute_angle * pi/180)),
    size = c(3, 2)
  )
  
  p <- ggplot2::ggplot() +
    ggplot2::geom_point(data = clock_df, 
                       ggplot2::aes(x = x, y = y), 
                       size = 3) +
    ggplot2::geom_text(data = clock_df, 
                       ggplot2::aes(x = x * 0.9, y = y * 0.9, label = label), 
                       size = 5) +
    ggplot2::geom_segment(data = hands_df,
  ...
  if (show_activation) {
    ...
    p <- p + ggplot2::geom_line(data = activation_df,
                               ggplot2::aes(x = x, y = y),
